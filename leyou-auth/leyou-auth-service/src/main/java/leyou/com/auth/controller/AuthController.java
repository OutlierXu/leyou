...
public class AuthController {
    ...
    /**
     * 登录请求路由到此（鉴权中心），鉴定用户有效性生成令牌token，写入cookie返回给浏览器
     * @param username
     * @param password
     * @param request
     * @param response
    ...
    public ResponseEntity<UserInfo> verifyUser(@CookieValue("Ly_TOKEN")String token,HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            ...
            //持久化cookie，每次有查询用户个人信息时就刷新cookie
            String newToken = JwtUtils.generateToken(userInfo, this.jwtProperties.getPrivateKey(), this.jwtProperties.getExpire());
            CookieUtils.setCookie(request, response, this.jwtProperties.getCookieName(), newToken, this.jwtProperties.getCookieMaxAge());
